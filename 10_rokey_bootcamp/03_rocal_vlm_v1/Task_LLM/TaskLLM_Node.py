...
class DualAgentControlCenter:
    def __init__(self):
        ...
        현재 주어진 YOLO 데이터에 사용자가 언급한 물체의 좌표가 없다면,[Task] 작업을 지시하기 전에 반드시 [Bring-1] 작업을 1단계 스텝으로 먼저 기획해.
        단, 여러 물체를 조작해야 할 경우 절대 모든 [Bring-1]을 1, 2단계에 몰아서 배치하지 마. 
        반드시 각 물체를 조작하는 [Task]의 '바로 직전 스텝'에 해당 물체의 [Bring-1]을 개별적으로 짝지어 배치해야 해.

        (올바른 흐름: A 스캔 ➔ A 조작 ➔ B 스캔 ➔ B 조작)
